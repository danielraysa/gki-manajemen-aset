            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-hover table-responsive" style="width: 100%">
                ...
                if (empty($_SESSION['temp_hapus'])) {
                  ...
                  $query = mysqli_query($koneksi,"SELECT d.ID_ASET, d.NAMA_BARANG, d.MASA_MANFAAT, d.TANGGAL_PEMBELIAN, DATE_ADD(d.TANGGAL_PEMBELIAN, INTERVAL (d.MASA_MANFAAT) YEAR) AS EXP_DATE, TIMESTAMPDIFF(YEAR,CURRENT_DATE(),DATE_ADD(d.TANGGAL_PEMBELIAN, INTERVAL (d.MASA_MANFAAT) YEAR)) AS DIFF, (SELECT COUNT(ID_ASET) FROM pemeliharaan_aset WHERE ID_ASET = d.ID_ASET AND STATUS_PEMELIHARAAN = 'Selesai') as JML_PEMELIHARAAN, d.NILAI_RESIDU, d.HARGA_PEMBELIAN FROM daftar_baru d WHERE d.STATUS_ASET = 'Aktif' HAVING JML_PEMELIHARAAN > 0 ORDER BY JML_PEMELIHARAAN DESC");
                }
                else {
                  $arr = array();
                  ...
                  $query = mysqli_query($koneksi,"SELECT d.ID_ASET, d.NAMA_BARANG, d.MASA_MANFAAT, d.TANGGAL_PEMBELIAN, DATE_ADD(d.TANGGAL_PEMBELIAN, INTERVAL (d.MASA_MANFAAT) YEAR) AS EXP_DATE, TIMESTAMPDIFF(YEAR,CURRENT_DATE(),DATE_ADD(d.TANGGAL_PEMBELIAN, INTERVAL (d.MASA_MANFAAT) YEAR)) AS DIFF, (SELECT COUNT(ID_ASET) FROM pemeliharaan_aset WHERE ID_ASET = d.ID_ASET AND STATUS_PEMELIHARAAN = 'Selesai') as JML_PEMELIHARAAN, d.NILAI_RESIDU, d.HARGA_PEMBELIAN FROM daftar_baru d WHERE d.STATUS_ASET = 'Aktif' AND d.ID_ASET NOT IN ('".implode("', '", $arr)."') HAVING JML_PEMELIHARAAN > 0 ORDER BY JML_PEMELIHARAAN DESC");
                }
                $a = 1;
                while($row = mysqli_fetch_array($query)) {
